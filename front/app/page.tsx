...
export default function HomePage() {
  ...
  return (
    <div className={`flex flex-col min-h-screen ${isDarkMode ? "bg-charcoal-900" : "bg-white"}`}>
      ...
      {/* Section avec image de Madagascar */}
      <section className={`py-32 relative overflow-hidden ${isDarkMode ? "bg-charcoal-900" : "bg-sage-50"}`}>
        <div className="container mx-auto px-6 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-16 items-center">
            <motion.div
              ...
              <div className="relative">
                <div
                  className={`absolute -inset-4 rounded-3xl blur-2xl ${
                    isDarkMode
                      ? "bg-gradient-to-r from-sage-600/20 to-coral-500/20 opacity-40"
                      : "bg-gradient-to-r from-sage-200 to-coral-200 opacity-30"
                  }`}
                ></div>
                <div
                  className={`relative rounded-2xl overflow-hidden shadow-2xl ${
                    isDarkMode ? "border border-sage-700/30" : ""
                  }`}
                >
                  <Image
                    src="/placeholder.svg?height=500&width=600&text=Madagascar+vue+satellite+-+Cartographie+territoriale+et+analyse+géospatiale"
                    alt="Madagascar vue satellite - Cartographie territoriale"
                    width={600}
                    height={500}
                    className="w-full h-auto object-cover"
                  />
                  <div
                    className={`absolute inset-0 ${
                      isDarkMode
                        ? "bg-gradient-to-t from-charcoal-900/50 via-transparent to-transparent"
                        : "bg-gradient-to-t from-sage-900/30 via-transparent to-transparent"
                    }`}
                  ></div>
                  <div className="absolute bottom-4 left-4 right-4">
                    <div
                      className={`backdrop-blur-sm rounded-lg p-3 ${
                        isDarkMode ? "bg-charcoal-800/90 border border-sage-700/30" : "bg-white/95"
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        <MapPin className="w-4 h-4 text-coral-500" />
                        <span className={`text-sm font-medium ${isDarkMode ? "text-white" : "text-charcoal-900"}`}>
                          Madagascar
                        </span>
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  <Badge
                    ...
                    className={`${
                      isDarkMode
                        ? "border-sage-700 text-sage-300 bg-sage-900/50"
                        : "border-sage-200 text-sage-700 bg-white"
                    }`}
                  >
                    Notre Territoire
                  </Badge>
                  ...
                  >
                    Madagascar
                    <br />
                    <span className={`font-bold ${isDarkMode ? "text-sage-400" : "text-sage-600"}`}>Numérique</span>
                  </h2>
                  <div className="w-16 h-0.5 bg-gradient-to-r from-sage-500 to-coral-400"></div>
                </div>
                <p className={`text-lg leading-relaxed ${isDarkMode ? "text-gray-300" : "text-charcoal-800"}`}>
                  Une île unique aux défis cartographiques exceptionnels. Madatlas développe des solutions innovantes
                  pour cartographier et analyser ce territoire d'une richesse extraordinaire.
                </p>
                <div className="grid grid-cols-2 gap-6">
                  <div
                    className={`text-center p-4 rounded-xl shadow-sm ${
                      isDarkMode ? "bg-charcoal-800 border border-sage-700/30" : "bg-white"
                    }`}
                  >
                    <div className={`text-3xl font-bold mb-2 ${isDarkMode ? "text-sage-400" : "text-sage-600"}`}>
                      587K
                    </div>
                    <div className={`text-sm ${isDarkMode ? "text-gray-300" : "text-charcoal-800"}`}>
                      km² de territoire
                    </div>
                  </div>
                  <div
                  ...
                  >
                    <div className={`text-3xl font-bold mb-2 ${isDarkMode ? "text-coral-400" : "text-coral-500"}`}>
                      28M
                    </div>
                    <div className={`text-sm ${isDarkMode ? "text-gray-300" : "text-charcoal-800"}`}>d'habitants</div>
                  </div>
                </div>
              </div>
            </AnimatedSection>
